...
class ThresholdPlayer(Player):
    ...
    def step(self, game: Game):
        ...
        to_open = p_for_min < self.open_tresh
        to_flag = p_for_max > self.flag_tresh
        no_moves = np.logical_not(np.any(to_open, axis=(1,2)) | np.any(to_flag, axis=(1,2)))
        p_no_moves = p_for_min[no_moves]
        to_open[no_moves] = p_no_moves <= p_no_moves.min(axis=(1,2), keepdims=True)+0.01
        no_moves = np.logical_not(np.any(to_open, axis=(1,2)) | np.any(to_flag, axis=(1,2)))
        if np.any(no_moves):
            raise Exception('No moves could be selected')
